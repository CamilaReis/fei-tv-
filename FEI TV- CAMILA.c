...
void fazerLogin() {
    ...
    if (encontrou) { // se encontar vai para filmes
        ...
        int op;
        printf("\nVoce nao possui cadastro. Deseja fazer cadastro? 1-Sim / 2-Nao (ESCOLHA 1 OU 2): ");
        scanf("%d", &op);
        setbuf(stdin, NULL);
        if (op == 1) {
            cadastrar();
...
void menuFilmes(char nomeUsuario[]) {
    ...
    if(escolha >= 1 && escolha <= 4) {
        ...
        printf("\n\ndeseja curtir o filme escolhido %s? 1-Sim / 2-Nao (ESCOLHA 1 OU 2): ", listaFilmes[escolha-1]);
        scanf("%d", &gostou);
        setbuf(stdin, NULL);
        if (gostou == 1) {
            printf("\nCurtido! Deseja adicionar aos favoritos? 1-Sim / 2-Nao (ESCOLHA 1 OU 2): ");
            scanf("%d", &fav);
            setbuf(stdin, NULL);
            if (fav == 1) {
